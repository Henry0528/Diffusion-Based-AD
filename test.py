...
def main(args: InferenceArgs, writer: SummaryWriter):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, states, gts) in enumerate(test_loader):
            ...
            diffmaps = run_inference_step(diffmap_blur, eval_scores, gts, i, fea_cat, model, noise_kind,
            ...
            anomaly_map_list.append(diffmaps)
            gt_list.append(gts)
            for pred, label in zip(diffmaps, states):
                labels_list.append(0 if label == 'good' else 1)
                predictions.append(torch.max(pred).item())
        threshold, auroc_score, auroc_pixel, pro = metric(labels_list, predictions, anomaly_map_list, gt_list)
