...
def metric(labels_list, predictions, anomaly_map_list, gt_list):
    ...
    resutls_embeddings = (
                (resutls_embeddings - resutls_embeddings.min()) / (resutls_embeddings.max() - resutls_embeddings.min()))
    ...
    auroc = auroc.reset()
    return thresholdOpt, auroc_score, auroc_pixel, pro
...
def compute_pro(masks, amaps, num_th=200):
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        df = pd.concat([df, pd.DataFrame({"pro": mean(pros), "fpr": fpr, "threshold": th}, index=[0])],
                       ignore_index=True)
